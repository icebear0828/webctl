...
|------|----------|----------|
| **ChatGPT** | `login`, `chat`, `models` | HTTP with token refresh (7-day TTL) |
...
```bash
# ChatGPT
webctl chatgpt login
webctl chatgpt chat "What is the meaning of life?"
webctl chatgpt chat "Follow up" --conversation-id <id> --model gpt-4o
webctl chatgpt models
...
| Site | TTL | Refresh |
|------|-----|---------|
| ChatGPT | 7 days | Access token auto-refresh via `/api/auth/session` |
...
| 网站 | 命令 | 策略 |
|------|------|------|
| **ChatGPT** | `login`, `chat`, `models` | HTTP + token 自动刷新（7 天 TTL） |
...
# ChatGPT
webctl chatgpt login
webctl chatgpt chat "生命的意义是什么？"
webctl chatgpt chat "追问" --conversation-id <id> --model gpt-4o
webctl chatgpt models
...
| 网站 | TTL | 刷新方式 |
|------|-----|----------|
| ChatGPT | 7 天 | 通过 `/api/auth/session` 自动刷新 access token |
